...
- Modify: `playwright.config.ts` (добавить `workers: 1`, см. Step 0 ниже — оба
  e2e-файла делят один процесс сервера, и без этого их тесты, гоняясь
  параллельными воркерами, вмешиваются друг в друга)

**Interfaces:**

- Consumes: реальный собранный сервер+клиент (как в `e2e/lobby.spec.ts`), реальный `packs/current.json`.

- [ ] **Step 0: Сериализовать e2e-воркеры**

`e2e/lobby.spec.ts` и `e2e/round.spec.ts` работают против одного и того же
процесса сервера — `playwright.config.ts`'s `webServer` один на весь
`testDir`, `Room` внутри процесса тоже одна на всё время его жизни
(`server/src/index.ts`, `new Room(...)` создаётся один раз). Participants,
однажды присоединившиеся, никуда не деваются из `Room.participants` —
только помечаются `connected: false`. Playwright по умолчанию гоняет разные
spec-файлы параллельными воркерами; без сериализации `lobby.spec.ts`'s
`'Ваня'` может всё ещё быть подключена (тест ещё не успел завершиться и
закрыть контекст) в момент, когда `round.spec.ts` жмёт «Начать игру» —
`Room.startGame()` (после фикса в Step 3 ниже) уже не тянет отключённых
участников в счётчики, но пока `lobby.spec.ts`'s тест физически не
завершился, «Ваня» какое-то время остаётся реально подключённой третьей
участницей с ненулевым шансом получить случайный первый ход (движок выбирает
`turnCounterId` случайно среди всех переданных `counterIds`). Добавить в
...
**Task 9, тот же паттерн «название теста обещает больше, чем проверяет тело», найден ревью (2026-08-04):** несмотря на явную установку при диспетчеризации именно на этот класс пробелов (Task 8 уже поймал такое же расхождение в `Player.tsx`), тесты плана для `Board.tsx` повторили его дважды: (1) тест «shows the correct answer and score on reveal» проверял только текст ответа, а не сам счёт, хотя название заявляло проверку счёта; (2) фазы `judging` не было ни в коде теста, ни где-либо в файле вообще, хотя код `Board.tsx` (правильно) показывает `correctAnswer` в обеих фазах — `judging` и `reveal` — используя одно и то же поле, без привязки к конкретной фазе. Сама реализация была верной с самого начала (проверено ревью сверкой с `Room.toGameStateView()`), но её никто не проверял на `judging` — незамеченная регрессия, привязывающая показ ответа обратно к `phase === 'reveal'`, прошла бы тесты незамеченной. Исправлено: тест на раскрытие переименован и дополнен проверкой `Ваня: 100`, добавлен отдельный тест на `judging`-фазу.

**Task 10, гонка и хрупкий локатор в собственном коде плана, найдены при повторной вычитке перед диспетчеризацией (2026-08-04):** после нескольких задач подряд, где ревью ловило то, что план не проверил сам, e2e-сценарий был перечитан заново перед запуском реализатора.

1. Определение «чей ход» строилось на голом `.isVisible()` сразу после клика по «Начать игру» — в отличие от locator-действий и `expect(...).toBeVisible()`, `.isVisible()` не ждёт появления элемента, а проверяет текущее состояние DOM немедленно. Сразу после клика WS-обновление могло ещё не долететь ни до одной из двух страниц, и голая проверка дала бы `false` для обеих — код молча (без исключения) приписал бы роль пикера неверной странице. Исправлено: `expect(async () => {...}).toPass()`, опрашивающий обе страницы, пока сетка не появится хоть у одной.
2. `expect(board.locator('p')).toContainText(/\?/)` — локатор `p` на табло матчит сразу несколько элементов одновременно (адрес лобби, «Выбирает …», текст вопроса), а `toContainText` на множественном локаторе либо требует массив ожиданий на каждый элемент, либо падает в strict-mode. Исправлено на `board.getByText(/\?/)` — вопросы в `packs/current.json` оканчиваются на «?», и на этом шаге сценария это единственный такой текст на экране, совпадение однозначно.

Оба — недосмотры при написании плана, не решения реализатора (задача ещё не диспетчеризована на момент находки).

**Критический регресс из фикса Task 4, найден реализатором Task 10 через реальный e2e (2026-08-04):** e2e-сценарий детерминированно зависал на «Ждём решения соперников» сразу после первого же голоса в судействе — реализатор не стал списывать это на флейковость и воспроизвёл причину через фейковые таймеры вне e2e, что и вскрыло настоящий баг.

Корень — именно тот фикс из Task 4 (см. запись «Task 4, баг в собственном коде плана, найден ревью» выше), который переносил сброс `gameTimeoutHandle`/`gameTimerDeadline` на уровень выше цикла по `effects[]`, чтобы устаревший дедлайн не оставался висеть при пустом `effects[]` на переходах `'reveal'`/`'round-end'` → `'selecting'`/`'game-end'`. Тот фикс был необходим, но недостаточно точен: он лечил один класс пустых `effects[]` (переход по истёкшему таймеру в фазу без своего таймера) ценой поломки другого класса — пустой `effects[]` от события `'vote'` во время судейства. `handleVote` в движке (Task 2) устроен намеренно: голос просто копится в `state.votes`, разрешение приходит только по `timer-expired: 'vote'` — сам `vote` никогда не эмитит `start-timer`. Безусловный сброс bookkeeping на любой пустой `effects[]` убивал уже тикающий таймер судейства при первом же вызове `room.vote(...)`, и ни один последующий голос его не переустанавливал — партия зависала навсегда, а не только «портился» отображаемый дедлайн, как в исходной находке Task 4.

Различие между двумя случаями пустого `effects[]` нельзя провести, глядя только на сам массив эффектов — нужно знать, что за событие его породило:

- Не-`timer-expired` событие с пустым `effects[]` (голос, отклонённое нажатие и т.п.) → про таймер ничего не меняется, трогать bookkeeping нельзя вообще.
- `timer-expired` с пустым `effects[]` (переход после РАСКРЫТИЯ/КОНЦА РАУНДА в фазу без таймера) → именно текущий таймер только что сработал сам, bookkeeping обязан обнулиться.

Исправлено: `dispatch()` передаёт в `applyEffects` признак `event.type === 'timer-expired'`; `applyEffects` трогает bookkeeping только когда либо пришли новые эффекты, либо истёк именно текущий таймер — `if (effects.length === 0 && !timerJustFired) return;` перед прежней логикой. Код плана (Task 4, `dispatch`/`applyEffects`) обновлён, добавлен регресс-тест, доказывающий, что голос не сбрасывает таймер и судейство всё равно разрешается по истечении. `room.ts` уже реализован и смержен раньше как часть Task 4 — фикс применяется к уже существующему файлу отдельным диспетчем, вне обычной последовательности задач, потому что баг всплыл только при реальном e2e с настоящими таймерами (Task 10), а не в модульных тестах с фейковыми таймерами, где сценарий с голосованием после входа в судейство просто не был написан.

**Task 10, локатор второго вопроса в сценарии, найден и исправлен реализатором (2026-08-04):** после починки регресса в `room.ts` тот же сценарий продвинулся дальше и упёрся в новую (не связанную с движком) проблему — на втором вопросе локатор `picker.getByRole('button', {name: /^\d+$/}).first()` иногда захватывал уже отвеченную, задизейбленную кнопку. Причина — `Player.tsx` (в отличие от `Board.tsx`, который целиком убирает разобранные вопросы через `.filter`) держит все кнопки сетки видимыми, дизейбля только уже отвеченные (`disabled={q.answered}`), так что `.first()` без фильтра по доступности иногда попадал не туда. Исправлено на `picker.locator('button:enabled', { hasText: /^\d+$/ }).first()`. Чисто тестовая правка, продакшен-код не менялся; заодно независимо подтвердила, что фикс регресса в `room.ts` действительно снял исходную зависшую партию — сценарий прошёл мимо места старого зависания и упёрся в это, новое и куда более мелкое.

**Task 10, реальная гонка между e2e-файлами, найдена ревью (2026-08-04):** `e2e/lobby.spec.ts` и `e2e/round.spec.ts` работают против одного и того же процесса сервера — `Room` внутри процесса одна на всё время его жизни (`server/src/index.ts`), а Playwright по умолчанию гоняет разные spec-файлы параллельными воркерами. `lobby.spec.ts` присоединяет «Ваня» и никогда не запускает игру; `Room.startGame()` (до фикса) тянула в счётчики **всех**, кто когда-либо присоединился, без фильтра по `connected` — так что если «Ваня» была ещё физически подключена (её тест ещё не успел завершиться и закрыть контекст) в момент, когда `round.spec.ts` жмёт «Начать игру», она становилась третьим счётчиком с реальным (≈1/3, `Math.random()` по числу счётчиков в `engine.ts`) шансом получить случайный первый ход — и тогда ни одна из двух реально управляемых страниц никогда не увидела бы сетку, а `expect(async () => {...}).toPass()` в сценарии опрашивал бы обе страницы вплоть до `test.setTimeout(90_000)`. Два «зелёных» прогона подряд у предыдущего реализатора ничего не доказывали — это была ровно та вероятностная гонка, которую и предполагается ловить именно так: несколько прогонов подряд периодически проходят «повезло», а не потому что гонки нет.

Исправлено двумя взаимодополняющими правками, ни одна из которых сама по себе не закрывает вопрос полностью:

1. **`Room.startGame()` (Task 4) теперь считает счётчиками и минимумом «два игрока» только реально подключённых сейчас участников**, а не всех, кто когда-либо заходил за время жизни процесса. Это не только тестовый костыль, а самостоятельно оправданное продуктовое исправление: игрок, зашедший в лобби и ушедший до начала партии, не должен оставаться фантомным счётчиком со случайным шансом на первый ход наравне с теми, кто реально играет — это соответствует общему принципу «Минимум для старта — два игрока» из исходной спеки, применённому к реально присутствующим, а не ко всем исторически заходившим. Добавлен регресс-тест.
2. **`playwright.config.ts` получает `workers: 1`**, сериализуя все e2e-файлы против общего сервера — без этого первое исправление снижает вероятность гонки (счётчиком станет только реально подключённый на момент старта участник), но не убирает её полностью: пока тест `lobby.spec.ts` физически выполняется, «Ваня» всё ещё реально подключена. Только вместе оба исправления делают гонку невозможной в принципе, а не просто маловероятной.

Рассмотренная и отклонённая альтернатива: оставить только фильтр по `connected`, полагаясь на то, что `lobby.spec.ts`'s тест (одно короткое присоединение плюс две быстрые проверки) статистически почти всегда успевает завершиться раньше, чем `round.spec.ts` дойдёт до собственного «Начать игру» (два последовательных join с сетевыми круговыми поездками). Отклонено — «почти всегда» и есть определение флейка; проект уже потратил в этой вехе несколько раундов именно на такие «почти всегда работает» гонки (Task 6, Task 10 сам по себе — гонка сообщений и локатор кнопки), и цена устранения (одна строка конфига) несопоставимо ниже цены оставить недетерминированный тест в постоянном наборе.

## Финальное ревью всей ветки (2026-08-04)

После всех 10 задач и всех внеплановых фиксов прошло финальное ревью на всю ветку целиком (`opus`, диапазон `6532dac..28eaec4`). Плану ничего не противоречило — все задокументированные выше отклонения проверены выборочно и подтверждены в коде без дрейфа. Ревью нашло пять новых находок уровня Important, ни одна не про повреждение данных, все — про то, что живая партия может застрять или процесс — упасть, если случится редкий, но реальный сценарий. Решения по каждой:

**1. `Room.startGame()` не гасит текущий таймер и не запрещает себя во время идущей партии; `server.ts`'s ветка `start-game` — единственная из игровых, не проверяющая `connections.get(ws)`.** Штатно недостижимо (кнопка «Начать игру» в `Player.tsx` рендерится только при `game === null`), но цена срабатывания — потерянная партия и упавший процесс (необработанный `TypeError` внутри уже не принадлежащего текущей игре таймера), а цена фикса — несколько строк. **Чиним.** `startGame()` гасит `gameTimeoutHandle` безусловно и отклоняет запуск, если `this.game` уже существует и не в `'game-end'`; `server.ts` требует `connections.get(ws)` как и остальные четыре игровые ветки.

**2. Когда все счётчики уже в `triedCounterIds`, вопрос всё равно переоткрывается на полные `QUESTION_TIMER_MS`, хотя нажать больше некому — 25 секунд мёртвого экрана, кнопка «Жать!» у всех активна и не делает ничего (`Room.buzz` возвращает `'ok'`, движок молча игнорирует событие).** Ни исходная спека, ни дизайн-документ этот случай не разбирают — пробел плана, не отступление от него. **Чиним.** В движке: если `triedCounterIds` после начисления штрафа покрывает все `state.scores`, сразу `revealQuestion(state, null)` вместо переоткрытия.

**3. Фаза `selecting` не имеет таймаута — прямое противоречие Global Constraints этого плана («Каждое состояние с ожиданием человека имеет таймаут») и исходной спеке («Ни одно состояние не ждёт человека бесконечно»).** Комментарий в `room.ts` называет это решением («спека не ограничивает время на выбор вопроса»), но нигде не зафиксирован как осознанное отклонение — раньше это было просто упущено. **Не чиним в этой волне, фиксируем как явное, осознанное отклонение вместо тихого пробела:** добавление таймера на выбор требует решить, кому переходит ход при истечении (тому же игроку? случайному? спека не отвечает), завести новую именованную длительность и её протестировать — это дизайн-решение, а не механический фикс, и его правильнее обсудить отдельно, чем протащить бездумно в волну финальных правок. Реальный риск ограничен: per спеку, обрыв связи — это норма, а не исключение (переподключение по токену), так что зависание наступает только если ушедший игрок никогда не вернётся за весь остаток вечера, а не при обычном «отошёл на минуту». Кандидат на Веху 2 или отдельный быстрый фикс по решению пользователя, не блокирует эту веху.

**4. `timerDeadline` уже приезжает клиенту (`GameStateView`, задокументирован в дизайне), но не рендерится ни на `Player.tsx`, ни на `Board.tsx` — обратного отсчёта не видно нигде.** Формально план выполнен (таблица «Клиенты» обратный отсчёт не требует явно) — это пробел дизайн-документа, не реализации. **Не чиним в этой волне** — визуальный компонент обратного отсчёта требует своего небольшого дизайна (секунды или прогресс-бар? где на экране?), и это ближе к полировке, сознательно отложенной вместе со звуком (см. «Звук на табло отложен» выше) — тот же класс решения, тем же принципом. Кандидат на быстрый последующий шаг после первой живой партии, когда будет понятно, насколько это ощущается недостающим на практике.

**5. Три реальных пробела в тестах.** **Чиним все три** — это ровно то, ради чего затевалось финальное ревью:

- Восстановление таймера из снапшота (`PHASE_TIMER` в конструкторе `Room`) не покрыто ничем — единственный механизм, не дающий партии зависнуть навсегда после падения сервера, и его никто не проверяет.
- Дизайн-документ прямо требует интеграционный тест на обрыв/возврат по токену **во время игры** — есть только версия для лобби, из скелета.
- `server.test.ts`'s `'plays a question from start-game through a correct answer'` доходит только до фазы `buzzed` и не проверяет то, что обещано в названии — тот же паттерн «название обещает больше, чем тело», пойманный дважды раньше в этой же вехе (Task 8, Task 9) и переживший обе итерации незамеченным здесь.

Дополнительно чиним из Minor-находок (дёшево, стоит того): `validatePack` не проверяет уникальность `id` вопросов, хотя движок использует их как глобальные ключи — дубль в написанном руками пакете тихо портит партию вместо ошибки валидации; и несогласованность табло/игрока в показе разобранных ячеек (табло удаляет через `.filter`, игрок дизейблит и оставляет — оба варианта валидны сами по себе, но тест на табло называется «shows... greyed out», а проверяет удаление) — переименовываем тест под то, что он реально проверяет, разметку не трогаем (дизайн явно допускает любой визуальный подход, стилей в проекте всё равно нет).

Остальные Minor-находки (недостижимый в текущем UI `falsestart`, форма входа при обрыве посреди партии вместо сохранения экрана игры, присоединившийся посреди партии не становится счётчиком но видит рабочие на вид кнопки, `Room.participants` никогда не уменьшается, расположение одного `import type` в тестовом файле) остаются отложенными — каждая либо унаследована от уже принятых решений скелета, либо явный кандидат Вехи 2, либо чисто косметическая.

### Пере-ревью волны фиксов: новый регресс в самом фиксе (2026-08-04)

Ре-ревью волны фиксов выше нашло новый критический регресс, внесённый самим фиксом находки 1: `startGame()`'s безусловный сброс `gameTimeoutHandle`/`gameTimerDeadline` был помещён **перед** проверкой `game-in-progress`, а не после — то есть даже **отклонённый** вызов `startGame()` (партия реально идёт) всё равно гасит работающий таймер. Для фаз `reveal`/`round-end`, которые продвигаются только по `timer-expired` (без действия игрока), это воспроизводит ровно ту же опасность, которую находка 1 должна была устранить: партия зависает без возможности восстановиться без перезапуска процесса. Новый регресс-тест на находку 1 не поймал это, потому что проверял отказ именно с фазы `'selecting'` — единственной фазы вообще без таймера, так что `timerDeadline` там и так `null` до и после, независимо от бага.

Ревью независимо воспроизвело проблему (`selectQuestion` → живой таймер → `startGame()` возвращает `game-in-progress`, но `timerDeadline` всё равно обнуляется и фаза никогда не продвигается по истечении 25с).

Пользователь подтвердил немедленное исправление одним точечным диспетчем (не автономная вторая волна фиксов — SDD-процесс финального ревью явно не предполагает второй волны, находка вынесена на решение и решение получено явно). **Исправлено:** сброс таймера перенесён на путь, который реально перезаписывает `this.game` (после обеих ранних проверок, непосредственно перед `this.game = createInitialState(...)`), а не выполняется безусловно в начале метода. Регресс-тест на находку 1 дополнен/заменён проверкой с фазы `question-open` (не `selecting`), которая утверждает, что таймер остаётся живым и всё ещё срабатывает после отклонённого `startGame()`.
