...
Две вещи, которые всплыли при разборе кода уже после того, как спека была закоммичена.
Читать до начала работы.

1. **Абзац спеки «Если удалённый пакет был открыт в редакторе, редактор закрывается на
   клиенте» описывает недостижимое состояние.** Редактор в `Admin.tsx` открывается
   единственной кнопкой «Редактировать», и она всегда открывает **активный** пакет
   (`setEditingFilename(activePackFilename)`, `Admin.tsx:931` — других присваиваний
   `editingFilename` в файле нет). Активный пакет удалить нельзя — это второе правило той же
   спеки. Значит «удалённый пакет открыт в редакторе» не наступает никогда. Эффект
   автозакрытия в плане **не реализуется** (YAGNI): у него не было бы теста, который его
   по-честному вызывает. Если позже появится редактирование неактивных паков — вернуть этот
   абзац вместе с ним.
2. **`title` на `disabled`-кнопке не показывается.** Отключённый элемент не получает
   mouse-событий, и всплывающая подсказка на нём не всплывает — то есть буквальное чтение
   спеки («кнопка выключена, с `title`») дало бы ровно ту беззвучную блокировку, ради ухода
   от которой этот `title` и заводился. Кнопка остаётся выключенной, но `title` вешается на
   обёртку `<span>` вокруг неё — тогда подсказка работает. Смысл спеки сохранён, механизм
   поправлен.
