...
**Проверить строку «Найденные сетевые адреса» в консоли при старте.** Если ноутбук имеет
больше одного сетевого интерфейса (VPN, VirtualBox, Docker — обычное дело), автовыбор может
взять не тот адрес молча, без ошибки — табло при этом выглядит полностью рабочим, просто
телефон никогда не достучится. Если выбранный адрес — не Wi-Fi, перезапустить с явным
адресом:

В PowerShell (основная оболочка) инлайн-переменная перед командой не работает — нужна
явная форма, она же удобнее для шага 6, потому что переменная остаётся в сессии до перезапуска:

```powershell
$env:LAN_HOST = '<ваш Wi-Fi IP>'
pnpm run start
```
...
```bash
LAN_HOST=<ваш Wi-Fi IP> pnpm run start
```
...
Выключить Wi-Fi на одном из телефонов **и продержать выключенным не меньше 10–15 секунд**
(не «на секунду») — сервер узнаёт об оборванном соединении через heartbeat с интервалом 5с,
двух тактов которого и требует таймаут; при более коротком отключении участник может вообще
не успеть пометиться отключённым, и это не будет означать, что что-то сломано. Табло должно
пометить участника как «(отключён)», не убирая его из списка. Включить Wi-Fi обратно —
телефон должен переподключиться сам (без перезахода на страницу), а табло — показать того же
участника снова подключённым, **под тем же именем, без дубликата**.

- [ ] **Step 6: Проверить восстановление по перезапуску сервера**

Остановить сервер (`Ctrl+C`), запустить заново (`pnpm run start`, не забыть `LAN_HOST`, если
он был нужен на шаге 1). Обновить страницу табло вручную — оба участника должны быть в списке
(помечены отключёнными, пока их телефоны не переподключатся сами) — это подтверждает, что
снапшот пережил перезапуск. Если после рестарта список пуст, а не «оба отключены» — смотреть
в консоли сервера строку «Снапшот ... повреждён»: это значит файл снапшота был битым
(например, из-за `Ctrl+C`, поймавшего запись на середине), и сервер корректно стартовал
с пустой комнатой вместо падения — это ожидаемое поведение деградации, не баг, но означает,
что предыдущее состояние действительно потеряно, а не просто не показано.
...
- Не рассылать `state` тому сокету, который только что присоединился, отдельной веткой
  условия — отклонено, но по другой причине, чем изначально записано здесь (неточность
  найдена и исправлена ревью задачи 4): `broadcastState` вызывается через `room.onChange`
  и физически не имеет ссылки на сокет, который вызвал изменение — исключить его из
  рассылки было бы возможно, только протащив эту ссылку через весь путь `Room → notify →
onChange`, что смешивает транспорт с моделью не меньше, чем вариант с отложенным
  `notify()` внутри `Room`, уже отклонённый выше по той же причине.
...
### Task 6 — два механических отклонения, без изменения поведения

В отличие от отклонений задачи 4, эти два не про поведение хука, а про несовместимость
буквального кода брифа с настройками именно этого окружения. Оба проверены напрямую по
диффу, не только со слов реализатора: `useRoomConnection.ts` не менялся вторым коммитом
вообще, значит второй фикс не мог задеть поведение хука.

**`client/src/useRoomConnection.test.ts` — `FakeWebSocket` без TS parameter properties.**
Буквальный код брифа объявлял `constructor(public url: string)`. У `client/tsconfig.app.json`
включён `erasableSyntaxOnly: true` (унаследовано из исходного Vite-скаффолда, до этого плана) —
он требует, чтобы стирание типов давало валидный JS без какой-либо дополнительной кодогенерации;
parameter properties этому не удовлетворяют, поскольку требуют настоящей кодогенерации
(создание поля и присваивание), а не только стирания аннотации. Заменено на обычное поле
класса и присваивание в теле конструктора — то же самое поведение в рантайме, синтаксис
приведён в соответствие с уже существующей настройкой проекта, не наоборот.

**`client/src/setupTests.ts` — полифил `localStorage`/`sessionStorage` для тестов.**
Начиная с Node 22, `localStorage` — встроенный глобал самого Node (нужен `--localstorage-file`,
без него бросает исключение при обращении). Vitest в режиме jsdom делит `globalThis` с `window`;
jsdom видит, что `localStorage` уже определён Node'ом, и не устанавливает свою рабочую
реализацию поверх. В тестовом окружении токен переподключения через `localStorage` падал не
из-за бага хука, а из-за этой гонки инициализации. Добавлен минимальный in-memory `Storage` в
`setupTests.ts`, подставляемый через `Object.defineProperty` до того, как тесты успевают
обратиться к `localStorage`.

**Что ещё рассматривалось и почему отклонено.**

- Использовать `jsdom` напрямую с явным флагом, отключающим глобал Node, — отклонено:
  требует трогать конфигурацию vitest/окружения ради частного случая, тогда как полифил
  в `setupTests.ts` — файл, который и так существует именно для настройки тестового
  окружения, ровно его назначение.
- Патчить сам `useRoomConnection.ts`, чтобы не полагаться на `localStorage` напрямую (например,
  через инъекцию хранилища как параметра, аналогично `wsFactory`) — отклонено: это реальное
  расширение публичного API хука ради проблемы, которая существует только в тестовом
  окружении и никогда не проявится в браузере (у браузеров есть собственный нативный
  `localStorage`, конфликт с Node-глобалом там просто не существует).
- Не чинить и оставить сломанные тесты — отклонено, очевидно: без этого сам факт хранения
  токена переподключения — центральной части этой задачи — остался бы непроверенным.

Это не поведенческие развилки, а привязка кода к реальному окружению, поэтому решение принято
без отдельного одобрения — записывается тем не менее, по тому же правилу, что и остальные
отклонения.

### Task 7 — ещё одно техническое отклонение: явный cleanup для RTL

`client/src/setupTests.ts` получил `afterEach(() => cleanup())` из `@testing-library/react`.
Причина: `vitest.config`/`vite.config.ts` не включает `test.globals: true` (тесты явно
импортируют `describe`/`it`/`expect` из `vitest`, не через глобальные переменные) — а
автоматический cleanup RTL полагается именно на глобальный `afterEach`, который без
`globals: true` не находит. Без этого фикса DOM от предыдущего теста оставался в дереве при
запуске следующего в том же файле — до задачи 7 это не проявлялось, потому что более ранние
клиентские тесты либо не рендерили DOM вовсе (`routing.test.ts` — чистая функция), либо
использовали `renderHook`, а не полноценный рендер компонента с несколькими `it()` подряд
в одном файле.

**Что ещё рассматривалось и почему отклонено.**

- Включить `test.globals: true` в конфиге — отклонено: меняет режим для всего клиентского
  пакета разом (все тесты перестали бы явно импортировать `describe`/`it`/`expect`), тогда
  как проблема локальна к одному конкретному поведению RTL. Правка в `setupTests.ts` — файле,
  созданном именно для подобных настроек тестового окружения, — уже, точнее и не меняет
  стиль остальных тестов.
- Не чинить и оставить возможную утечку DOM между тестами — отклонено: в файле с несколькими
  `it()` это источник трудноуловимых ложных срабатываний/непрохождений в будущем, ровно
  тогда, когда меньше всего ожидаешь.

Файл затронут только этим — `Player.tsx` в этом коммите не менялся.

### Task 9 — сброс снапшота комнаты перед каждым e2e-прогоном

`server/src/index.ts` пишет состояние комнаты в `room-snapshot.json` относительно cwd и
загружает его при старте процесса. `playwright.config.ts`'s `webServer.command` поднимает
новый процесс сервера на каждый прогон `pnpm run test:e2e` — если файл остался от прошлого
запуска (участник «Ваня» уже в комнате), `Room.join('Ваня')` во втором прогоне вернёт
`name-taken`, и набор проходит один раз, а затем детерминированно падает на каждом
следующем запуске.

**Почему так.** В `webServer.command` перед `pnpm run build && pnpm run start` добавлен шаг
`node e2e/reset-snapshot.mjs`, удаляющий файл снапшота до того, как сервер вообще стартует.
Заодно `room-snapshot.json`, `test-results/`, `playwright-report/`, `playwright/.cache/`
добавлены в `.gitignore` — это закрывает и отдельную, ранее отложенную находку ревью
задачи 4 («`room-snapshot.json` не игнорируется»), не только эту.

**Что ещё рассматривалось и почему отклонено.**

- `globalSetup`-хук Playwright, удаляющий файл перед прогоном — отклонено и **проверено**,
  а не предположено: Playwright запускает `webServer` раньше, чем `globalSetup`, так что
  к моменту, когда `globalSetup` успел бы сработать, сервер уже загрузил старый снапшот.
- Чистить `room-snapshot.json` внутри самого `server/src/index.ts` при старте всегда —
  отклонено: это меняет реальное поведение сервера (снапшот и существует именно для того,
  чтобы переживать перезапуск — задача 10 живьём проверяет ровно это), а нужно было
  почистить только тестовое окружение, не поведение продакшен-кода.
- Оставить как есть — отклонено: e2e-сюит, падающий детерминированно со второго запуска,
  бесполезен как регрессионный тест.

### Финальное ревью всей ветки — семь находок, все исправлены до задачи 10

После того как все 9 кодовых задач прошли собственное ревью, отдельный финальный прогон
ревью на всю ветку целиком нашёл то, что ни одно ревью отдельной задачи не могло увидеть —
проблемы, видимые только когда всё собрано вместе. Две из них предсказуемо провалили бы
живую проверку на задаче 10, если бы их не поймали заранее:

1. **`pickLanAddress` может выбрать виртуальный адаптер вместо реального Wi-Fi** —
   подтверждено живьём именно на машине разработки (VirtualBox-адрес `192.168.56.1`
   перечисляется раньше Wi-Fi). QR-код на табло указывал бы в никуда, и это не давало бы
   никакой видимой ошибки. Исправлено: `LAN_HOST` как явный override плюс лог всех найденных
   кандидатов при старте (`server/src/network.ts`, `index.ts`). Задача 10 обновлена —
   проверять строку «Найденные сетевые адреса» и запускать с `LAN_HOST`, если нужно.
2. **Ни один тест не проверял путь «снапшот → рестарт → reconnect»** — headline-заявление
   этой вехи оставалось непроверенным автоматически. Добавлен тест в `room.test.ts`.
3. **Битый снапшот мог не дать серверу стартовать вообще** — а задача 10 буквально требует
   `Ctrl+C` посреди работы. Загрузка снапшота обёрнута в `try/catch` с деградацией до пустой
   комнаты; `main()` получил `.catch()`; добавлен `httpServer.on('error')` на `EADDRINUSE`.
4. **Не было heartbeat** — без него сервер узнаёт об оборванном сокете через TCP-таймаут
   (минуты), а не за секунды, и «(отключён)» из задачи 10, шаг 5, скорее всего никогда бы
   не появился за то время, что задача просит держать Wi-Fi выключенным. Это была находка не
   про реализацию, а про сам план — heartbeat никогда явно не запрашивался ни одной задачей,
   но задача 10 полагалась на его эффект. Добавлен стандартный ping/pong-хартбит (интервал 5с)
   в `server.ts`. Задача 10, шаг 5 переписан — держать Wi-Fi выключенным 10–15с, не «секунду».
5. **Тестовые файлы сервера дублировались через `server/dist`** — задача 9 сделала это
   постоянным эффектом (каждый e2e-прогон теперь пересобирает `dist`). Исправлено одной
   строкой в `server/tsconfig.json` (`exclude`).
6. **`ping.ts`/`ping.test.ts` — мёртвый код** с самой задачи 4, удалён.
7. **Седьмая находка, не входившая в исходный список — нашлась при живой проверке фикса 3,
   не по чтению кода.** `WebSocketServer`, созданный с `{ server: httpServer }`, подписывается
   на `'error'` httpServer'а и переэмитит его на себя; без собственного слушателя на `wss`
   `EventEmitter` на необработанный `'error'` бросает исключение — то есть фикс 3 в
   исходной формулировке не решал бы задуманную проблему вообще: занятый порт всё ещё падал
   бы сырым стеком. Добавлен `wss.on('error', ...)` рядом с `httpServer.on('error', ...)`
   в `server.ts`, с регрессионным тестом, проверенным на то, что он реально ловит откат
   (без фикса — таймаут, с фиксом — 10мс).

**Что намеренно оставлено вне объёма.** `writeSnapshot` по-прежнему пишет одним `writeFile`
без temp+rename — обрезанный файл всё ещё физически возможен, фикс 3 лишь делает его
безвредным для старта (сервер деградирует до пустой комнаты вместо падения). Атомарная запись
убрала бы саму возможность обрезки, но это изменение продакшен-поведения персистенции, не
запрошенное этим ревью — вынесено отдельной задачей.

Полный отчёт с командами и живыми прогонами — в `.superpowers/sdd/2026-08-03-walking-skeleton/final-fix-report.md`
(это scratch-пространство SDD-процесса, будет удалено по завершении; ключевые факты
продублированы здесь и в тексте задачи 10 намеренно, чтобы не потеряться вместе с ним).

### Пост-ревью — `writeSnapshot` пишет атомарно (temp + rename)

Пункт «Что намеренно оставлено вне объёма» финального ревью выше явно откладывал этот
фикс: `writeSnapshot` писала снапшот одним `writeFile` прямо в целевой путь, так что обрыв
записи (`Ctrl+C`, падение процесса — ровно то, что производит живая проверка на задаче 10)
физически мог оставить на диске обрезанный JSON. Фикс 3 финального ревью сделал такой файл
безвредным для _старта_ сервера (деградация до пустой комнаты вместо падения), но сама
возможность обрезки оставалась — участники молча исчезали на том самом рестарте, который
снапшот должен переживать.

**Что изменилось.** `writeSnapshot` (`server/src/snapshot.ts`) теперь пишет во временный файл
`${path}.tmp`, а затем переименовывает его поверх целевого пути через `rename()`. `rename` —
атомарная операция и на POSIX, и на Windows, поэтому целевой путь в любой момент виден либо
полностью старым, либо полностью новым — обрезанного промежуточного состояния для читателя
больше не существует.

**Регрессионный тест.** `server/src/snapshot.test.ts` мокает `writeFile` так, чтобы он реально
писал на диск только часть данных (имитация обрыва прямо посреди записи), а затем бросал
исключение — и проверяет, что после этого `readSnapshot` по-прежнему возвращает старое, полностью
валидное состояние. Тест написан по TDD: до фикса он падал именно на том, что `readSnapshot`
бросал `SyntaxError` на обрезанном JSON (мок писал прямо в целевой путь, как это делала старая
`writeSnapshot`); после фикса мок пишет только во временный файл, целевой путь не трогается до
`rename`, и тест проходит.

**Почему решение принято сейчас, а не оставлено отклонённым.** Пользователь явно запросил
именно этот фикс отдельным поручением после финального ревью и подтвердил подход (temp + rename)
до реализации.
