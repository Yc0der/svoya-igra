...
describe('place-bid', () => {
  function biddingState(): EngineState {
    return selectAuction(createInitialState(AUCTION_PACK, ['p1', 'p2', 'p3']))
      .state;
  }
  ...
  it('is a no-op for a bid not exceeding the current highest', () => {
    const afterFirst = reduce(biddingState(), {
      type: 'place-bid',
      counterId: biddingState().auctionTurnCounterId!,
      amount: 150,
    }).state;
...
describe('resolveVote — вопрос-аукцион', () => {
  function auctionJudging(
    ...
    let state = selectAuction(
      createInitialState(AUCTION_PACK, counterIds, hostId),
    ).state;
    const order = state.auctionOrder!;
    state = reduce(state, {
      type: 'place-bid',
  ...
  it('credits the winner with their winning bid amount, not the pack price, on a correct answer', () => {
    const judging = auctionJudging('judge', ['p1', 'p2', 'judge'], 350);
...
`Phase` — добавить `'auction-bidding'` после `'question-open'`:
