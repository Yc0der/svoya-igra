...
- [ ] **Step 3: Добавить новые поля в `GameStateView` (protocol.ts)**

`Room.toGameStateView()`'s объявленный тип возврата — `GameStateView | null` из
`server/src/protocol.ts`; без этого шага `room.ts` не типизируется — поля, которые Step 5
добавит в возвращаемый объект, не существуют на самом интерфейсе ещё. (Полные варианты
`ClientMessage` для `place-bid`/`pass-bid` остаются в Task 5 — они room.ts не нужны, только
`GameStateView`.)

`server/src/protocol.ts`, `GameStateView` — новые поля после `exclusiveAnswererParticipantId`:
...
- [ ] **Step 6: Написать падающий тест миграции снапшота**
...
- [ ] **Step 8: Реализовать в `snapshot.ts`**
...
```bash
npx tsc --noEmit
npx eslint src/room.ts src/room.test.ts src/snapshot.ts src/snapshot.test.ts src/protocol.ts
cd ..
git add server/src/room.ts server/src/room.test.ts server/src/snapshot.ts server/src/snapshot.test.ts server/src/protocol.ts
...
- Consumes: `Room.placeBid`/`Room.passBid`, `Room.toGameStateView()`'s новые поля,
  `GameStateView`'s новые поля (все — Task 4, `GameStateView` уже расширен там, поскольку
  `room.ts` иначе не типизируется).
- Produces: `ClientMessage` получает `{ type: 'place-bid'; amount: number }`,
  `{ type: 'pass-bid' }`.

- [ ] **Step 1: Расширить протокол**

`server/src/protocol.ts`, `ClientMessage` — новые варианты после `select-question`
(`GameStateView` уже содержит нужные поля — добавлены в Task 4):
