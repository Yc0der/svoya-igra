 ...
 * функцией appendAtEnd. Это лечение порядка, а не косметика. Анкету ниже
 * машинного раздела больше не создаёт никто — upsertPlayerSection дописывает
 * её в конец анкетной части, — но файл заявлен правимым руками, и лежащие на
 * дисках файлы помнят версию, дописывавшую анкету в абсолютный конец. Замена
 * «на месте» закрепила бы такой порядок навсегда: каждый следующий пересчёт
 * переписывал бы раздел там же, где он и был, — ниже анкеты. Вырезание с
 * дописыванием в конец возвращает раздел статистики под анкеты на первом же
 * game-end.
 */
