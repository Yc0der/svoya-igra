...
export class Room {
  ...
  // 2026-08-21): 270мс подобраны уже на настоящем табло (живая проверка
  // 2026-08-21) — первые 200мс были выбраны на мониторе разработчика и
  // после телевизора оказались чуть резковаты. Не часть RoomState,
  // сбрасывается при перезапуске сервера в дефолт. 270 — то же значение, что
    ...
    return {
      ...
      currentQuestion: currentQuestionData
        ? {
            ...
            // замораживается на входе в question-reveal, как revealMs.
            // ВАЖНО: это не бесплатно. animationDelay каждой буквы
            // (useTextReveal.tsx) считается от старой длительности и не
            // меняется задним числом, а animationDuration берётся живьём —
            // при смене значения буква, которая в этот момент на середине
            // проявления, даёт заметный скачок прозрачности (потемнеет
            // обратно, если fadeMs увеличили, или мгновенно станет
            // непрозрачной, если уменьшили; проверено в браузере через
            // getComputedStyle). Затронуты буквы в окне fadeMs от момента
            // смены — при 200мс это 3-4 буквы, при выкрученных до 1000мс —
            // до полутора десятков. Это принято осознанно: крутит ручку и
            // одновременно смотрит на экран только тот, кто её настраивает
            // (design.md, «Длительность проявления настраивается в
            // админке»), а тратить кадр на скачок — цена за то, что новое
            // значение видно сразу, а не только со следующего вопроса.
