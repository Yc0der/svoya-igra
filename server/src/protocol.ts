...
export interface GameStateView {
  phase: Phase;
  // Замороженный на время партии ведущий (EngineState.hostId), НЕ то же
  // самое, что лобби-флаг StateMessage.hostParticipantId — тот меняется в
  // лобби до старта и может ещё указывать на кого-то, кто был отмечен
  // ведущим, но на момент фактического старта партии оказался отключён
  // (design.md, «Ведущий»: «во время партии роль зафиксирована»). Клиент
  // обязан считать себя ведущим по ЭТОМУ полю, пока идёт партия, а не по
  // лобби-флагу — иначе кнопки панели ведущего рисуются, но требуют
  // авторизации, которой у нажавшего фактически нет, и молча ничего не
  // делают.
  hostId: string | null;
  roundIndex: number;
  grid: {
