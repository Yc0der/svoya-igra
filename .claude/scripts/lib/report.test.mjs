...
test('fmtTokens на границе порядка уходит вверх, а не печатает четыре разряда', () => {
  // 999 500 округляется в «k» до 1000 — это читается хуже, чем «1.0M», и ломает
  // выравнивание колонок в таблице сессий, где под сокращение заложены три разряда.
  assert.equal(fmtTokens(999_500), '1.0M');
  assert.equal(fmtTokens(999_499), '999k');
  assert.equal(fmtTokens(999_950_000), '1.0G');
  assert.equal(fmtTokens(999_949_999), '999.9M');
});
