...
- `server/src/` — движок и сервер. `engine.ts` — чистая машина состояний игры, не знает про
  часы и сокеты. `room.ts` — комната: игроки, очки, таймеры, вывод сигналов звука.
  `server.ts` — HTTP и ws. `protocol.ts` — типы сообщений между клиентом и сервером.
  `pack.ts`/`packs.ts` — формат и загрузка пакетов вопросов. `history.ts` — история партий.
  `audioAssets.ts` — сканирование `audio/`, `audioSettings.ts` — настройки звука и очередь их
  записи. Рядом с каждым файлом `*.test.ts`.
- `client/src/` — `Board.tsx` (экран на телевизоре), `Player.tsx` (телефон игрока),
  ...
  Звук: `audio.ts` — движок воспроизведения вне React, `useBoardAudio.ts` — связка табло с
  ним, `VolumeSlider.tsx` — ползунок громкости для табло и админки.
...
- `audio/` — звуки табло: сигналы с фиксированными именами и `music/`. В git только
  `README.md`; сервер сканирует папку один раз при старте.
